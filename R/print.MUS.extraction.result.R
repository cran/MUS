print.MUS.extraction.result <- function(x, print.title=TRUE, print.planning=FALSE, style="default", use.pander=FALSE, ...){
	...
	tbl[1,] = c("Sample", nrow(x$sample), value(sum(x$sample[,x$col.name.book.values])),
		percent(sum(x$sample[,x$col.name.book.values]) / x$book.value)	)
	tbl[2,] = c("High Values", nrow(x$high.values), value(sum(x$high.values[,x$col.name.book.values])),
	...
	tbl[4,] = c("Population", nrow(x$data), value(sum(x$data[,x$col.name.book.values])),
